    ...
    addPinsToMapView = (pins) => {
        for (let i = 0; i < pins.length; i++) {
            ...
            for (let y = 0; y < pins[i].pinCategory.length; y++) {
                if (pins[i].pinCategory[y] === "Niche") {
                    this.catNiche.push(pins[i]);
                }
                if (pins[i].pinCategory[y] === "Will Visit Again") {
                    this.catReturn.push(pins[i]);
                }
                if (pins[i].pinCategory[y] === "Oddity") {
                    this.catOdd.push(pins[i]);
                }
            }
			...
			`,
                { className: "text-center justify-content-center maps-font" }
            ).openPopup();
